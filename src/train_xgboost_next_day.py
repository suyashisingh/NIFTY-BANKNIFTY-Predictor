...
# --- Paths ---
DATA_PATH = "data/processed/nifty_engineered_features2024.csv"
MODEL_DIR = "models"
os.makedirs(MODEL_DIR, exist_ok=True)

if not os.path.exists(DATA_PATH):
    raise FileNotFoundError(f"Input file not found: {os.path.abspath(DATA_PATH)}")
...
feature_cols = [str(col) for col in feature_cols]  # Ensure all feature names are strings

# --- Train/test split ---
split_date = '2024-07-01'  # Adjust as needed
...
for col in non_numeric_cols:
    try:
        ...
        print(f"Dropping column {col} due to conversion error.")
...
if cols_to_drop:
    print("Dropping columns due to excessive NaNs:", cols_to_drop)
X_train = X_train.drop(columns=cols_to_drop)
X_test = X_test.drop(columns=cols_to_drop)

# --- Impute remaining NaNs with column mean ---
print("NaN count per column in X_train after conversion and dropping:")
...
feature_names_path = os.path.join(MODEL_DIR, 't_day_feature_names_2024.txt')
...
model.fit(X_train, y_train)  # Fit with DataFrame to store feature names as strings
